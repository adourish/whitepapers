...
Seven AI coding assistant platforms were evaluated across performance, reliability, browser automation, skill sharing, agentic governance and orchestration, permission models, FedRAMP compliance, and cost. The field splits into two decision contexts: **standard development environments** and **FedRAMP-regulated government networks**.

### Key Takeaways

1. **The full dev loop is the primary selection criterion.** Only **Claude Code** and **Devin** complete the write → test API → open browser → inspect UI → fix loop autonomously and reliably. Cursor and Windsurf Pro handle code + terminal well and **can** open a browser via Playwright, but it fails frequently and requires retries. GitHub Copilot and Amazon Q have no browser automation. Windsurf FedRAMP has Playwright but it fails frequently, compounded by the known working-folder bug. **Claude Code leads**: Playwright MCP + Chrome Extension + CLI, three access modes (CLI, VS Code, Mobile), 9/10 full dev loop.
...
11. **Only Claude Code serves every role from a single platform.** Developers via CLI and VS Code, QA via Playwright, architects via MCP/skills, PMs/BAs via Claude.ai desktop, executives via desktop + mobile. Every other tool is developer-only or developer-primary. Selecting a tool that spans roles compounds the productivity investment.

12. **Skill sharing is a premier force-multiplier — only Claude Code makes it first-class.** Skills are plain markdown CLAUDE.md files in git — a BA-trained doc skill, QC Playwright skill, or architect estimation workflow is instantly available org-wide. No other tool has a structured cross-role mechanism: Cursor uses per-repo .cursorrules, Windsurf Pro project-scoped workspace rules, Devin has no persistent library, GitHub Copilot offers org-level custom instructions only.

13. **Claude Code leads on performance, governance, and junior developer safety.** Subagents parallelize workloads in a single session (analyze + test + implement simultaneously), reducing wall-clock time on complex tasks. Claude Code runs natively in VS Code or a plain terminal with significantly lower CPU overhead than IDE-embedded tools; Windsurf FedRAMP, as a fork of the commercial release, carries additional CPU overhead from its less-optimized architecture. The hook system intercepts every AI lifecycle event — PreToolUse, PostToolUse, UserPromptSubmit, SubagentStart/Stop — enabling approval gates, audit trails, and CI/CD signals. **This is critical for junior developers**: agentic AI without guardrails can overwrite files, delete branches, or execute destructive scripts unchecked. Hooks give tech leads governance control over what autonomous AI can do, making agentic AI safe at the junior level. No other tool has a comparable governance or multi-agent model.

14. **Amazon Q Developer is only competitive in AWS-centric stacks.** Its model quality trails the field for general-purpose coding but adds meaningful value for CDK, CloudFormation, and Lambda patterns under FedRAMP.

15. **Cost-per-task — not monthly sticker price — is the right unit for comparison. The spread is 33×:**

- 🥇 **Windsurf Pro** — $0.75/day (1.0× baseline) · ~$0.30–0.60/task · 500 credits/mo · ✅ instant $10/250-credit top-ups, no expiry
- **GitHub Copilot / Amazon Q** — ~$0.95/day (1.3×) · ~$0/task (flat) · unlimited completions
- **Cursor** — $1.00/day (1.3×) · ~$0.12–0.20/task · 500 fast req/mo · graceful fallback
- **Claude Code Pro** — $1.00/day (1.3×) · ~$0.67–1.00/task · weekly window exhausts fast · ❌ no top-up
- **Claude Code Max 5x** *(recommended)* — $5.00/day (6.7×) · ~$0.67–1.00/task · ❌ no top-up — secondary tool required when window exhausts mid-sprint
- **Claude Code API — Sonnet** — ~$0.45/task (pay-per-use) · cheaper than Max 5x below ~200 tasks/month
- **Windsurf FedRAMP** — ~$4.00/day (5.3×) · high/task · ❌ 4–8 wk contract amendment to expand
- **Devin** — $25.00/day (**33.3×**) · ~$25–50/task · 50–100× more than CC API on Sonnet
...
|---|---|---|
| Standard development | **Claude Code** | 8.9/10 across all criteria; best agentic ecosystem |
...
| **Salesforce development** | **Claude Code** or **Windsurf Pro** | Best for orchestrating Apex/LWC push-pull to/from Salesforce orgs; script execution handles sfdx/sf CLI automation. Windsurf FedRAMP viable for regulated Salesforce environments. Agentforce Vibe (Salesforce-native) is notable for pure Salesforce flows but not general-purpose. GHC, AQ, Cursor, Devin do not integrate well with Salesforce org workflows. |
...
| **Junior Developer** | **Claude Code** (VS Code Ext) or **Windsurf Pro** | Full agentic support from day one — juniors benefit most from AI doing the heavy lifting; VS Code Extension or Cascade provides guided agentic in a familiar IDE. **Hooks are critical for junior safety**: Claude Code hooks allow tech leads to set approval gates on destructive operations (file deletes, branch resets, script execution), preventing unguarded agentic AI from causing irreversible damage | Cursor (model-flexible IDE alternative); GitHub Copilot (lowest friction if GHE is in place) |
...
| ⭐ **Agentic Governance & Orchestration** | **8%** | **10** | **3** | **1** | **1** | **1** | **1** | **1** |
...
Salesforce: best choice for Apex/LWC orchestration"]
    ...
    subgraph T2["Tier 2 — Capable but Limited (5.0–5.4)"]
        DV["Devin — 5.4
Strong autonomy + CLI
